 ...
 *  Missing or empty `capabilities` is treated as "unknown" — the action runs
 *  and the server enforces. Pairs with the same convention in
 *  filterCommandsByCapability. */
export function ensureCapabilityOrExit(
  capabilities: string[] | undefined,
  verb: string,
): void {
  if (!capabilities || capabilities.length === 0) return;
  if (capabilities.includes(verb)) return;
  const err = new Error(
